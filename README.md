...
## 진행 상황
> 마지막 업데이트: 2026-09-23 · 현재 작업: DB 테이블 설계

### 1단계: 프론트엔드
- [x] 화면 설계 및 구현 (12개 / 12개)

### 2단계: 백엔드 환경 구성
- [x] Spring Boot 프로젝트 구성 (도메인 기준 패키지 구조)
- [x] Docker Compose 로컬 개발 환경 (MySQL 8.0, Redis 7)

### 3단계: DB 설계
- [x] 회원 · 지갑 · 계좌 테이블 설계
- [ ] 상품 테이블 설계 (1:N 사이즈 · 색상 분리)
- [ ] 주문 · 거래내역 · 정산 테이블 설계
- [ ] ERD 작성
...
### 5단계: 동시성 제어
- [ ] 지갑 잔액 차감: 낙관적 락 (`@Version`)
- [ ] 상품 재고 차감: Redis 분산 락
- [ ] 주문 시 지갑 · 재고 · 정산 트랜잭션 원자성 보장
- [ ] 동시성 테스트
