...
export const getHistoryBadge = (statement: string) => {
    ...
    if (statement === '결제실패')
        return <span className="bg-[#FFE4E4] text-red-400 px-2 py-1 rounded-full text-xs">결제실패</span>;
    if (statement === '처리중')
...
export const getOrderBadges = (orderStatus: OrderStatus) => {
    ...
    if (orderStatus === 'paymentFailed')
        return <span className="rounded-full bg-[#FFE4E4] text-red-400 px-2 py-1 text-xs">결제실패</span>;
};